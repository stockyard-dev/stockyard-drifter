...
async function load(){var r=await fetch(A+'/connections').then(function(r){return r.json()});items=r.connections||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.driver||'').toLowerCase().includes(q)||(i.dsn||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No connections.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1">';
h+='<div class="item-title">'+esc(i.name)+'</div>';
if(i.driver)h+='<div class="item-sub">'+esc(i.driver)+'</div>';
...
if(i.dsn)h+='<span>'+esc(String(i.dsn))+'</span>';
if(i.status)h+='<span>'+esc(String(i.status))+'</span>';
if(i.table_count)h+='<span>'+esc(String(i.table_count))+'</span>';
if(i.last_query_at)h+='<span>'+esc(String(i.last_query_at))+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
h+='</div>';});
document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/connections/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:'',driver:'',dsn:'',status:'',table_count:'',last_query_at:'',notes:''};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' CONNECTION</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name||'')+'" ></div>';
h+='<div class="fr"><label>Driver</label><input id="f-driver" value="'+esc(i.driver||'')+'" ></div>';
h+='<div class="fr"><label>Dsn</label><input id="f-dsn" value="'+esc(i.dsn||'')+'" ></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status||'')+'" ></div>';
h+='<div class="fr"><label>Table Count</label><input id="f-table_count" value="'+esc(i.table_count||'')+'" ></div>';
h+='<div class="fr"><label>Last Query At</label><input id="f-last_query_at" value="'+esc(i.last_query_at||'')+'" ></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),driver:document.getElementById('f-driver').value.trim(),dsn:document.getElementById('f-dsn').value.trim(),status:document.getElementById('f-status').value.trim(),table_count:document.getElementById('f-table_count').value.trim(),last_query_at:document.getElementById('f-last_query_at').value.trim(),notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/connections/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/connections',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
