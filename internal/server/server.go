package server
import ("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-drifter/internal/store")
...
func(s *Server)routes(){
    ...
    s.mux.HandleFunc("DELETE /api/flags/{key}",s.handleDeleteFlagByKey)
    s.mux.HandleFunc("POST /api/flags/{key}/toggle",s.handleToggleFlag)
    s.mux.HandleFunc("GET /api/eval",s.handleEval)
    s.mux.HandleFunc("GET /api/eval/all",s.handleEvalAll)
    s.mux.HandleFunc("GET /",s.handleUI)
}
func(s *Server)handleHealth(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-drifter"})}  
func(s *Server)handleVersion(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"version":"0.1.0","service":"stockyard-drifter"})}  
